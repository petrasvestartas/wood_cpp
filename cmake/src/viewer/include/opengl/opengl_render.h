...
namespace opengl_render
{
	...
	inline void render(bool (*function)())
	{
		...
		const std::string &relativePath = "src\\";
		std::filesystem::path basePath(std::filesystem::current_path().string());
		basePath = basePath.parent_path(); // Move up to the project's base directory if needed
		basePath /= relativePath;		   // Append the relative path to the resources
		std::string asset_directory = basePath.string();
